...
def main() -> None:
    ...
    LOGGER.info("WallDetection pipeline initialized on %s", detector.device)
    if detector.device != "cuda":
        raise SystemExit(
            "CUDA device not available. This script requires a GPU for inference."
        )
    ...
    LOGGER.info("Completed processing %d image(s).", len(image_paths))
